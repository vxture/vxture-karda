...
env-aware `stack_root`/`deploy_dir`, and the `karda-beta` OIDC client.

## Release

- [ ] `git tag vX.Y.Z && git push origin vX.Y.Z`, then approve the pending
      `production` deployment. During the development phase this is the only
      release path - with no beta tier live yet, that approval gate carries the
      full weight of pre-deploy scrutiny (TD-001 records the accepted interim
      risk). `git tag beta-YYYYMMDD.N` becomes available once beta is wired.
