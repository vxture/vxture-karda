...
| Owner-transported secrets: `DEPLOY_SSH_KEY`, `DEPLOY_KNOWN_HOSTS`, `ENV_FILE_BASE64`, `KARDA_DB_SVC_PASSWORD` | first `v*.*.*` tag can deploy | todo - only karda can receive them, only the owner can produce them. `ENV_FILE_BASE64` must also carry `OIDC_CLIENT_SECRET`, `PLATFORM_API_URL` and `PLATFORM_INTERNAL_AUTH_TOKEN` (see `80-liaison/50-2607230957`) |
