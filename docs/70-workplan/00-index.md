...
§11 第 2 条已拍板(owner 2026-08-26):**KD-211 抽取是独立一趟,不作管线第六档**。

**尚待拍板(§11 第 4、5 条)**:KD-206 是否穿透副本、断言能否跨库。
...
| `#155` | DS 升 9.0.7 | 已合 |
| `#156` | **KD-211 落地**:`incr/0008` 任务种类 + 抽取 pass + 独立 tick;`failure_class` 分出 `unavailable`,驻留不再被谎报成「配额」 | 已合 |
| `#158` | CI 三处真实浪费:codeql 占一半分钟数、secret-scan 每个提交扫两遍、纯文档 PR 跑全套 | 已合 |
| `#159` | 对端需求登记册(`260` §11)+ 「待对端必须带 issue 号」机器检查 | 已合 |
| `#160` | `karda.browse`——工具面十二件,Knowledge API 扩面收口 | 已合 |
| `#161` | 消费 Atlas 契约制品,把「靠人抄码表」这条路关掉 | 已合 |
...
| `#169` | **接上 §7 的写侧回收**:`sweepUngrounded` 建好测过却**零调用者**,现接入治理扫描端点;作用域「全部」改为必须明写 | 已合 |
| `#170` | 收尾审计:把批次 15 全部导出过一遍查同类缺口,剩三个「已建未接」并登记(`140` §11.3) | 已合 |
| `#171` | 技术债登记册审计——三条描述的是「还没建」,而东西早已建好;并加状态/索引硬门 | 已合 |
| `#172` | U 级库属主转移的特权路径,结清 TD-005,列锁一点没放宽 | 已合 |
| `#173` | `150-page-architecture`:外壳的规则齐了,页面的规则一条没有 | 已合 |
| `#174` | KD-215 域卡片搬去首页、导航栏收成标准菜单;并按 150 对齐页面 | 已合 |
| `#175` | **未授权的四档化**:「用不了」分四档带原因码,界面说清缺什么、谁在哪补 | 已合 |
| `#176` | 每份卡住的文档自己说卡在哪;并给目录加 markdown 硬门 | 已合 |
...
| `#191` | 待复验队列 DS 表化;`/pipeline/tasks` 判为看板不收(判据入 150 §3.6) | 已合 |
| `#192` | 卡尔达数字接真实口径(workspace-read,三个面同一读模型);**设置页整页重设计**(KD-224:左导航即概要) | 已合 |
| `#193` | 供给与加工数字全面转真(页级开关:有过真流量才转真,demoOps 随之翻转) | 已合 |
| `#194` | TD-014 关闭:cookie 承载服务端 locale,16 页标题跟随语言,<html lang> 首字节即正确 | 已合 |
| `#195` | **TD-010 关闭**:db-init 顺序改为 baseline -> 97 -> incr/* -> 98。v0.10.0 首次 db-init 死在 98 授权 source_mode(同类第三次);一次性 Postgres 18 重演生产轨迹证明修复 | 已合 |
...
**次序更正(owner 2026-08-28)。** 此前工作计划把「Atlas 端点授权」写成阻塞项,而
**授权是平台在管理面对一个已注册产品做的页面操作**——次序只能是 发布上线 → 平台注册
与工作区开通 → 授端点。把它当成开发或上线的前提,等于要求平台给一个还没上线的东西
授权,是一个闭不上的环。它阻塞的只是**运行时的检索结果**,不阻塞任何一步开发或发布。

由此定下的纪律:**产品必须在未授权状态下正常构建、发布、运行**,把缺的那一格具体地
说出来。落地为 `kb/processing/unavailable.ts` 的四档原因(依据是**修复人不同**:运维
没配 Atlas / 平台没开通工作区 / 平台管理面没授端点 / 库的模型锁指向 Atlas 没有的模型),
以及首页按档渲染「缺什么 + 谁在哪补」。详见 `60-operations/40-run-atlas-endpoint-grants` §0。

**批次 15 的代码面已经收口**:断言模型 v2(五表)、抽取缝与存储、抽取调用与独立一趟、Knowledge API 扩面十二件工具,全部合入并各自过真库探针。

**§11 五条全部结清**(owner 2026-08-27 落槌最后两条):

- **KD-212 回收穿透副本**——实例与快照都要跟着回收。与 KD-204 不冲突:**KD-204 管时间**
  (没人来删就不因时间流逝而自动清),**KD-212 管请求**(有请求就必须清)。两条措辞相触
  是因为都用了「清除」却没写触发条件。
  **工程后果:血缘列 `knowledge_base.origin_kb_id` 已在库里,但应用侧一处都没读过**——
  结构就绪,缺的是一次血缘感知的回收。已登记 **TD-015**,并在设计稿里写明:**在它落地
  之前,回收对副本是不生效的**。「已裁定穿透」与「已实现穿透」之间的差别,恰好是客户
  会问到的那个差别。
- **KD-213 断言不跨库**——`kb_id` 非空,跨库共享走发布。理由不是省事:一条断言若同属
  两库就**同时受两套治理策略管辖**,而两套策略可以要求相反的事。`incr/0006_*` 已是这个
  形状,**无需改结构**。

**运行仍等 `vxture-platform#55`** 的产品轴端点授权(操作单 `60-operations/40-*`)——没有 `embedding/default`,加工管线提交不了,**一份文档都进不了可检索状态**。抽取真正产出断言仍等 `vxture-platform#55`,
