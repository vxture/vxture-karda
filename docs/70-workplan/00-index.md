...
**只剩两条待你拍板**(设计稿 §11 第 4、5 条),都不阻塞代码:

1. **KD-206 回收是否穿透副本**(实例/快照)——登记册里 KD-206 的未答子问题在断言层的同形,
   与 KD-204(archived 不自动清除)措辞相触。**这条我没有倾向**:它取决于「删除请求」
   在合同上承诺到什么程度,那是业务判断不是工程判断。
2. **一条断言能否跨库**——**倾向:否**,`kb_id` 非空。跨库共享走发布,不走断言复用;
   否则一条断言会同时受两套治理策略管辖,而两套策略可以要求相反的事。
