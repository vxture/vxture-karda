...
- **020 + 025 (health)**: `buildHealthIdentity()` imported directly from the
  ...
  re-implementation - never hand-roll one), called from the liveness
  ...
  git tag/sha/date in `.github/workflows/build.yml`. Karda inherits these
  unchanged, so `/api/health` conforms out of the box - leave them alone.
