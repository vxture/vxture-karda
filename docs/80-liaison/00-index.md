...
| `100-2607240931-karda-atlas-capability-requirements.md` | 2607240931 | atlas line | field-level requirements for A1 embedding / A2 parsing models / A3 rerank, as design input while Atlas builds them. Flags the hard constraints karda cannot yield (version-locked embedding, 100-candidate rerank under 400ms, 429 that distinguishes throttle from quota) and the unlock order A1 > A3 > A2 | **answered 2026-07-27 by the atlas line (issue #70)** - G1 429/quota final, A2.3 affinity resolved, A3.3 P95 still uncommitted (design "may lower N"), A1 batch+version-pin adopted; priority A1 > A3 > A2 acknowledged (no date). See Received |
...
| `vxture-atlas/docs/80-liaison/10-2607241030-atlas-reply-to-karda-capability-requirements.md` (delivered as karda issue **#70**) | 2607241030 | **atlas line** | the atlas line's formal reply, answering `100` (G1, A3.3) and effectively `140`/`141`. **Contract facts now final/confirmed:** (G1) `429 {code:RATE_LIMITED, retryAfterMs}` + `Retry-After` = back-off-and-retry; `403 {code:QUOTA_EXHAUSTED, resetAt}` = suspend `suspended_quota`, do not retry; shared envelope `{code, message, requestId}`, status+code never reused. (A4) endpoint `POST /model-platform/chat` + RS256/JWKS auth **confirmed** (matches karda's built client). (A2.3) parse affinity resolved - Atlas + karda co-located on worker-02, design same-host. (A1) batch API + version-pinned `modelCode` (no silent latest-drift) adopted. **(A3.3)** rerank P95 @ 100 still **uncommitted** - design "unknown, may lower N", Atlas will benchmark + report once real. **New (§6):** `GET /model-platform/models?tenantId=` tenant-filtered model list + optional `taskProfile` on chat/embed/rerank/parse (`404 TASK_PROFILE_NOT_ROUTABLE` on no match) - karda's own asks, additive. **Correction (§5):** platform token-exchange has been live since 2026-07-12; the one open gate for an `aud=atlas` token is whether **Atlas's product registration ran in production** (`product.products` row + OIDC client mapping) - confirm with the PLATFORM line | **received 2026-07-27.** Closes `100`; answers `140`/`141`. Corrects karda's `141` claim "blockers fully cleared" - the true residual is the platform-side Atlas product-registration run (letter to platform line pending). Unblocks the `taskProfile` auto-adapt wiring + model-list UI (karda task 6) |
...
an independent repo, so the endpoint / model / S2S details were requested direct
from the atlas line in `140`, corrected in `141`, and **answered 2026-07-27 (issue
#70)**. Confirmed: endpoint `POST /model-platform/chat` + verify-side RS256/JWKS
(matches karda's built client), host `worker-02:3100` (co-located), and the model
is selected via `taskProfile` or the tenant-filtered list `GET
/model-platform/models?tenantId=` (#70 §6). karda has built the S2S token-exchange
caller (aud=atlas, service mode) and wired it dynamically into the A4 client, so
the code is ready. **The one honest residual** (correcting `141`'s "blockers fully
cleared"): per #70 §5 the platform's token-exchange mechanism is live (since
2026-07-12), but an `aud=atlas` token specifically needs **Atlas's product
registration to have run in production** (`product.products` row + OIDC client
mapping) - the code is merged platform-side, but whether it has been applied
(`db-init`) must be confirmed with the platform line. So a live call waits on that
one confirmation, then setting `ATLAS_BASE_URL` + the ask task-profile on the host.
...
registry table, not a draft letter - which is why the Atlas host was actionable
ahead of the letter. The S2S auth contract is no longer a draft either: the atlas
line confirmed it formally in issue #70 (RS256/JWKS verify, `taskProfile`/model-list
selection), leaving only the platform-side Atlas product-registration run to verify.
