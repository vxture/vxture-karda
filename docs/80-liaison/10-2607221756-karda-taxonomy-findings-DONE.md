...
> **状态**：**已闭环（2026-07-22）**——D1–D4 及 F1/F4/F5/F7/F8 已于 070 **批 5** 全部落地；
> 平台回函 `vxture/docs/80-liaison/20-2607221900-taxonomy-070-revision-reply.md`；本仓校准见 §6
...
|---|---|---|---|
| D1 | 同意 | 070 §3 kind 集合扩为 `{data,design,ops,product}`，显式允许 `product` 类省 domain 段；`ops` 视使用意图去留。**零文件改名**——现有 11 篇 `product_*` 即刻合法 | ✅ 批 5 已落 |
| D2 | 同意，补机检 | platform 的 `check-docs-numbering.mjs` 补目录名检查 + 具名例外 | ✅ 批 5 已落（`NUMBERED_DIR` + `DIR_EXEMPTIONS`）。29 个未编号子目录按"键控 / 历史遗留"两组入例外表，未强制改名——引用面过宽，改造收益/风险不对称，例外表注明只收缩不新增 |
| D3 | 同意，归还 `220` | `20-specs/220-vxtpl/` 迁出产品号段；`220` 归 karda | ✅ 批 5 已落：vxtpl → `940-vxtpl`（层外号段，非产品矩阵成员）。**`220` 已实际空出** |
| D4 | 同意，补充 | 070 §3 补：产品仓行使授权后须固化仓内约定文档 + 接入机检 + 回报平台线 | ✅ 批 5 已落，并引本仓 `docs/00-meta/10-docs-convention.md` 为参照实现 |
...
**本仓落地校准（2026-07-22，对齐 070 批 5 后执行）**：

1. **TD 登记表迁址**——批 5 的 §4（F1 校准）把路径钉成 `60-operations/10-tech-debt.md`，本仓原先
   放在 `60-operations/00-index.md` 内，**这是校准后新出现的偏差**，已迁出为独立文件；
   `deploy.yml` 里 TD-001 的指向同步更新。
2. **`DIR_EXEMPTIONS` 改为路径键**——本仓原用裸目录名（`decisions` / `rebuild`），会连带放行树中
   任意位置的同名目录，与本函 F5 批评的 README basename 全局豁免属同一类洞；照 platform 实现改为
   docs 相对路径，并加了负向用例（`40-implementation/decisions/` 现被拦下）。
3. **采纳序列/键控子目录模型**——本仓约定 §4 从"两个具名例外"改写为批 5 的两类模型，并预置了
   将来 karda DB schema 目录按 `30-design/db/schemas/`（键=表名）登记为键控子目录的先例。
4. **约定文档头部记录对齐版本**（批 5），§8 偏差登记结论：**零偏差**；§1 继承条款表按批 5 条款重写。
5. `220` 已空出，`docs/70-workplan/` 的 blocked 项解除。
