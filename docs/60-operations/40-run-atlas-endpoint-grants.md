...
**说在两处**:首页的可用性条按档汇总(只在不正常时出现),资产页**每一份卡住的文档
自己也说一遍**——「加工暂停:`embedding/default` 端点未授权给产品 karda…」。两处读的
是同一组句子(`_i18n/messages/states.ts` 的 `blocker*`),不是各写一份。

需要两处是因为它们回答的是**不同的人的不同问题**:首页问「这套东西现在能不能用」,
资产页问「我传的这一份怎么了」。只做首页,点进库里看某份文档仍旧是哑的。
