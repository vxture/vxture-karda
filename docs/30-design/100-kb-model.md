...
| 配置组 | 配置项 | v1 默认 |
|---|---|---|
| 来源 | **来源模式**(`source_mode`:自建 / 采集)——见 §7.1 | 自建(`owned`) |
...
### 7.1 来源模式:这个库的真相住在哪(KD-218,owner 2026-08-30)

一个库要么**自建**(`owned`,内容由属主放进来,真相就在这里),要么**采集**
(`synced`,内容从外部来源同步进来,真相在源头)。

这条区分产品**早就在用**,只是没有地方让库把它说出来:

| 用到它的地方 | 规则 |
|---|---|
| 治理 | `governanceApplies`:同步内容默认豁免,因为真相在源头(§5.2) |
| 删除 | 连接器的删除信号只覆盖它同步进来的那部分;`revokeCascade` 只墓碑该绑定的文档(I4) |
| 对账 | 对账扫描不能碰手工上传的文件 |

三条规则各自独立地记着同一件事,而答案只能**逐条**从 `document.source` 推——于是
「这个库可不可信」没有一个答案,只有 N 个。`source_mode` 是那个答案的位置。

**它是默认,不是约束。** 模式决定这一页的形状与各项默认,但**不禁止**往采集库里手工
补一份:「上游同步的规章 + 几份自己写的解读」是真实场景,禁止它只会把人赶去建第二个
库,而那样就丢掉了「这两者是一件事」这个事实。补进来的那一份因为 `synced=false` 而
**照常纳入治理**——那是对的,它的真相确实在这里。

界面上的落点(`150-page-architecture` §3.4):自建库没有「外部来源」那一格;采集库以
来源与同步状态为主角,并把其中手工放进来的内容标为**本地补充**。模式改变**不搬内容**:
采集转自建之后,已同步的文档还在,只是从此没有人再替它们更新。
...
| # | 决策项 | 说明 | 倾向建议 | 状态 |
|---|---|---|---|---|
| 1 | Entry 编辑权与 U 级库发布的交互 | ws_published 后的 U 级库,Entry 是否仍仅属主可编辑 | 发布改可见性不改写入权 | **已定 KD-003** |
| 2 | filterable 字段上限值 | 每库 filterable 字段数(含系统维度) | 16 | **已定 KD-001** |
| 3 | v1 预置 ContentTemplate 清单 | 首批平台条目模板 | FAQ / 术语 / SOP 卡 | **已定 KD-002**(联动 KD-201) |
| 4 | archived 保留策略 | 归档内容的存储计费与保留期 | 占存储、不自动清除 | 待 owner KD-204 |
...
- 2026-08-30:新增 §7.1 来源模式(KD-218),对应 DDL `incr/0009_kb_source_mode.sql`。
