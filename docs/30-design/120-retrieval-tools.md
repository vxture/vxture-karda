...
| # | 决策项 | 说明 | 倾向建议 | 状态 |
|---|---|---|---|---|
| 1 | 召回参数基线 | N=50/空间、精排池 100、top_k=10 | 按 §2 起步,上线后以召回测试数据调 | ⏳ |
| 2 | 可见集失效事件契约 | 平台事件通道的消息格式与投递语义 | 平台侧(product_310)定义,Karda 出消费需求清单 | ⏳ |
| 3 | 平台内容配额保护 | live P 包内容是否设结果占比上限 | 不设,精排统一裁决(§2 已裁,留复议位) | ⏳ |
| 4 | 工具面边界 | 发布/晋升/删库等是否永不进工具面 | v1 不进;v2 视 agent 治理场景(如"帮我发布")再议,须带用户确认交互 | ⏳ |
| 5 | filters 入参能力 | 业务 filterable 字段的查询算子集 | v1 仅等值与 in;范围查询(数值/时间)视需求 v1.x | ⏳ |
