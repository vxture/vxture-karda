...
| # | 决策项 | 倾向建议 | 状态 |
|---|---|---|---|
| 1 | v1 首批开放哪些连接器 | Arda（内部、现成）+ 一个纯 `poll` 型外部源作为框架验证 | ⏳ |
| 2 | `reconcile=none` 的源是否允许接入 | 允许但标注"无对账能力"，且不允许承载敏感内容 | ⏳ |
| 3 | 轮询周期下限 | 按连接器声明，框架设全局下限防滥用 | ⏳ |
| 4 | 第三方凭据托管形态 | 复用平台密钥设施还是 karda 自建加密存储；涉 L0，需另议 | ⏳ |
