...
| 值班台 | steward dock | 右侧那块 pane,400px。**它住着 Karda Super Agent**(见 §1.4);「值班台」是这块**地方**的名字,不是那个 agent 的名字 | ARIA `<aside>`;Xcode inspector |
...
**区分**:仓内仍用 **值班台 / steward dock** 指那块**外壳区域**(§1 词汇表)。区域是家具,
