...
导航栏与智枢**贴外边距**,自己不加边距;内容区是唯一的例外,额外加 16px 内衬,于是内容距两侧 pane 为 `32 + 16 = 48px`(2026-08-25 前是 `24 + 32 = 56px`)。理由不变:两侧是卡片面,内容也是卡片面,只隔一个栏间距时读起来是挤在一起的(owner 2026-08-24)——改的是这段留白由谁出,从内衬为主改成栏间距为主。
...
内容区实际宽度 = 视口 − 48(外边距)− 280(导航栏)− 400(智枢)− 64(两处栏间距)− 32(内衬):
...
2026-08-25 的框架调整对**两侧展开**恒定 **−4rem**(智枢 +80、栏间距 +16、
...
的「两侧展开默认三列」是在 320px 智枢下算出来的)。收起任一侧即回到三/四列
