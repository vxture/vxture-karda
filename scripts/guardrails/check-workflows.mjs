 ...
 * making the file invalid YAML. All five required checks passed - none of them
 * reads a workflow file - and the break only surfaced when GitHub refused to
 * dispatch the workflow, reporting that it "does not have a workflow_dispatch
 * trigger". A workflow that cannot be parsed is indistinguishable from one that
 * does not exist, which is the worst way to discover a CI change is broken:
 * silently, at the moment you need the pipeline.
 *
...
function scan(name, text) {
  ...
  //    it as a new top-level key instead of script text. A quote-counting
  //    heuristic was tried first and cried wolf on `sed "s/'/''/g"`, which is
  //    perfectly valid - so match the structure, not the punctuation.
  lines.forEach((l, i) => {
