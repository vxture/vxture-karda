#!/usr/bin/env bash
# On-host deployment lifecycle for the karda production stack. Invoked
...
ROOT="$(cd "$DEPLOY_DIR/.." && pwd)"     # /srv/md0/karda
ENV_FILE="$ROOT/etc/.env"
COMPOSE_FILE="$DEPLOY_DIR/docker-compose.yml"

# Product code: from the environment (CI passes PRODUCT_CODE), else the
# karda literal an instantiated product repo already replaced.
PRODUCT_CODE="${PRODUCT_CODE:-karda}"
