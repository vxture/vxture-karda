...
// task detail) -> right rail (three-tier queues + steward failure-rate alert
      ...
      <div className="grid grid-cols-2 gap-lg xl:grid-cols-4">
        <Card className="py-md border-t-medium border-t-primary">
          <CardContent className="flex h-full flex-col gap-2xs px-lg">
            <span className="font-mono text-[10px] tracking-widest text-muted-foreground">今日吞吐</span>
            <span className="flex items-baseline gap-sm">
              <span className="font-mono text-title-sm">{data.throughput.docsToday}</span>
              <span className="text-xs text-muted-foreground">docs · 端到端 P95 {data.throughput.p95Seconds}s</span>
            </span>
          </CardContent>
        </Card>
        <Card className="py-md border-t-medium border-t-primary">
          <CardContent className="flex h-full flex-col gap-2xs px-lg">
            <span className="font-mono text-[10px] tracking-widest text-muted-foreground">队列深度</span>
            <span className="flex items-baseline gap-md font-mono text-body-sm">
              <span>
                ...
                <span className="text-xs text-muted-foreground">交互</span>
              </span>
              <span>
                {data.queueDepth.sync} <span className="text-xs text-muted-foreground">同步</span>
              </span>
              <span>
                {data.queueDepth.bulk} <span className="text-xs text-muted-foreground">批量</span>
              </span>
            </span>
          </CardContent>
        </Card>
        <Card className="py-md border-t-medium border-t-warning-border">
          <CardContent className="flex h-full flex-col gap-2xs px-lg">
            <span className="font-mono text-[10px] tracking-widest text-muted-foreground">失败与挂起</span>
            <span className="flex items-baseline gap-md font-mono text-body-sm">
              <span>
                <span className="text-title-sm text-destructive-text">{data.failures.permanent}</span>{" "}
                <span className="text-xs text-muted-foreground">失败驻留</span>
              </span>
              <span className="text-warning-text">
                {data.failures.quota} <span className="text-xs text-muted-foreground">配额挂起</span>
              </span>
            </span>
          </CardContent>
        </Card>
        <Card className="py-md border-t-medium border-t-success-border">
          <CardContent className="flex h-full flex-col gap-xs px-lg">
            <span className="font-mono text-[10px] tracking-widest text-muted-foreground">阶段 P95 · 秒</span>
            <span className="flex h-[26px] items-end gap-xs">
      ...
      <div className="flex flex-col gap-lg xl:flex-row">
        ...
        <div className="flex min-w-0 flex-1 flex-col gap-md">
          <div className="flex items-center gap-xs">
            ...
            <span className="rounded-full border border-border px-3.5 py-1 text-xs text-muted-foreground">
              配额挂起 {data.counts.suspended}
            </span>
            <span className="rounded-full border border-border px-3.5 py-1 text-xs text-muted-foreground">
              失败驻留 {data.counts.failed}
            </span>
            <span className="ml-auto text-[11px] text-muted-foreground">按进入时间 · 幂等键去重</span>
          </div>
            ...
            {data.tasks.map((t) => (
              ...
              >
                <span className="flex min-w-0 flex-1 flex-col gap-2xs">
                  ...
                  <span className="truncate font-mono text-[10.5px] text-muted-foreground">{t.detail}</span>
                </span>
                <StageDots dots={t.dots} />
                <span className={`w-[7.5rem] shrink-0 text-right text-xs ${STATUS_CLASS[t.statusTone]}`}>
                  {t.statusLabel}
                </span>
              </Link>
            ))}
        ...
        <div className="flex w-full shrink-0 flex-col gap-md xl:w-[22rem]">
          <Card className="py-md">
            <CardContent className="flex flex-col gap-sm px-lg">
              <div className="flex items-baseline justify-between">
                ...
                <span className="font-mono text-[10px] text-muted-foreground">{data.orgConcurrency}</span>
              </div>
              ...
              <div className="border-t border-dashed border-primary/10 pt-sm text-[11px] leading-relaxed text-muted-foreground dark:border-primary/20">
            ...
            <Card className="py-md border-t-medium border-t-warning-border">
              <CardContent className="flex flex-col gap-xs px-lg">
                <div className="flex items-center gap-sm">
                  ...
                  <span className="ml-auto font-mono text-xs text-warning-text">{data.alert.rate}</span>
                </div>
          ...
          <Card className="py-md">
            <CardContent className="flex flex-col gap-sm px-lg">
              <span className="font-mono text-[10px] tracking-widest text-muted-foreground">失败分类 · 24H</span>
              <div className="flex flex-col gap-xs text-body-sm">
                <span className="flex items-center gap-sm">
                  <span className="size-2xs rounded-full bg-muted-foreground" />
                  瞬态(退避重试中)
                  <span className="ml-auto font-mono text-xs text-muted-foreground">{data.failures.transient}</span>
                </span>
                <span className="flex items-center gap-sm">
                  <span className="size-2xs rounded-full bg-destructive" />
                  永久(驻留待修正)
                  <span className="ml-auto font-mono text-xs text-destructive-text">{data.failures.permanent}</span>
                </span>
                <span className="flex items-center gap-sm">
                  <span className="size-2xs rounded-full bg-warning" />
                  配额(恢复自动续)
                  <span className="ml-auto font-mono text-xs text-warning-text">{data.failures.quota}</span>
                </span>
              </div>
