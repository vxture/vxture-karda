...
// service/product (karda), and honest fallbacks flow through end to end when
// build env is absent - no fabrication (025 section 6).
...
test("GET wires service/product from BRAND and returns the full identity block", async () => {
  await withNoProvenanceEnv(async () => {
    ...
    assert.equal(body.service, "karda-app");
    assert.equal(body.product, "karda");
